...
function startServer() {
    const CMD1 =
        ...
        server_config.root_password +
        ' -d ' +
        server_config.docker_image;
    ...
    exec(CMD1)
        ...
        .catch((err) => {
            if (err.stderr.includes('is already in use by container')) {
                console.log(
                    'container is already created. starting the existing container'
                );
                return exec('docker start ' + server_config.container_name);
            } else {
                console.log(
                    'container setup has failed. please contact developer with the following error info:'
                );
                console.log(err);
            }
        });
